 ...
 public:
  ...
  void run() {
    ...
    for (size_t k = 0; k < measurements.size(); ++k) {
      const Key i = static_cast<Key>(k + 1);
      const Key j = static_cast<Key>(k + 2);
      graph.add(BetweenFactor<Pose3>(i, j, measurements[k], odometryNoise));
    }
    ...
    if (use_mocap_) {
      ...
      const noiseModel::Diagonal::shared_ptr mocapNoise =
          noiseModel::Diagonal::Sigmas(
              (Vector(3) << mocap_std_dev, mocap_std_dev, mocap_std_dev).finished());

      for (const auto& meas : mocap_measurements) {
        const int pose_idx = meas.first;
        const Point3& mocap_point = meas.second;
        graph.add(MoCapPosition3Factor(pose_idx, mocap_point, mocapNoise));
      }

      // --- Gauge fix: strong full Pose3 prior at x1 (anchors orientation AND position)
      {
        using gtsam::noiseModel::Diagonal;
        gtsam::Vector6 anchor_sigmas;
        // [rx ry rz tx ty tz] : small sigmas -> strong prior
        anchor_sigmas << 1e-3, 1e-3, 1e-3, 1e-3, 1e-3, 1e-3;
        auto strongAnchor = Diagonal::Sigmas(anchor_sigmas);
        graph.add(gtsam::PriorFactor<gtsam::Pose3>(1, gtsam::Pose3(), strongAnchor));
      }
      // End 3b.
    }
