...
class SfmNode : public rclcpp::Node {
 public:
  ...
  SfmNode() : Node("sfm_node") {
    ...
    // is.
    // Insert code below:
    // Reasonable prior on the first pose (small rot/pos sigmas)
    auto priorNoise = noiseModel::Diagonal::Sigmas(
        (Vector(6) << 1e-3, 1e-3, 1e-3,   1e-1, 1e-1, 1e-1).finished()); // [rx,ry,rz, tx,ty,tz]
    ...
    for (const auto& meas : measurements) {
      const size_t cam_i = meas.first;            // camera index
      const Keypoint& kp = meas.second;           // (landmark idx, pixel)
      const Key lkey = Symbol('l', kp.lmk_idx_);  // landmark key 'l'
      const Key xkey = Symbol('x', cam_i);        // pose key 'x'

      graph.add(ProjectionFactor(
          kp.kpt_coords_,           // z: measured pixel (u,v)
          measurementNoise,         // pixel noise (Isotropic 1 px)
          xkey, lkey,               // variable keys
          K));                      // intrinsics
    }
    ...
    // Insert code below:
    auto pointPrior = noiseModel::Isotropic::Sigma(3, 0.1); // ~10 cm
    graph.add(PriorFactor<Point3>(Symbol('l', 0), landmarks[0], pointPrior));
