    ...
    switch (pose_estimator_) {
      ...
      case 3: {
        ...
        if (!use_ransac_) {
          ...
          // Solve R,t that minimize sum || R * p1_i + t - p2_i ||^2
          ...
          // ==== With RANSAC using OpenGV's 3D-3D problem (ARUN) ====
...
// Helpers: Kabsch (Arun) on arbitrary inlier set
...
// Convert clouds to Eigen vectors for convenience
...
for (int it = 0; it < iters; ++it) {
  // Sample minimal set of 3 unique indices
  int i1 = uni(rng), i2 = uni(rng), i3 = uni(rng);
  // Make them distinct; try a few times
...
// Optional: refine with all inliers of the best model
if (best_inliers >= 3) {
  std::vector<double> res;
